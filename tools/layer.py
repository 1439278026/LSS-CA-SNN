...
class NeuNorm1d(base.MemoryModule):
    def __init__(self, in_channels, timestep, k=0.9, shared_across_channels=False):
        super().__init__()
        self.register_memory('x', 0.)
        self.k0 = k
        self.k1 = (1. - self.k0) / in_channels ** 2
        if shared_across_channels:
            self.w = nn.Parameter(Tensor(1, timestep))
        else:
            self.w = nn.Parameter(Tensor(in_channels, timestep))
        nn.init.kaiming_uniform_(self.w, a=math.sqrt(5))
...
class TAB_Layer1d(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(self.time_steps):
            # xt = x[:,0:(t+1),...]
            y = x[:,0:(t+1),...].transpose(1, 2).contiguous()  # [N,T,C,H,W] ==> [N,C,T,H,W], put C in dim1.
            y = self.bn_list[t](y)
            y = y.transpose(1, 2).contiguous()  # [N,C,T,H,W] to [N,T,C,H,W]
            # y_res[:,t,...] = y[:,t,...].clone()  # Only slice the t-th
            y_res.append(y[:,t,...].clone())  # Only slice the t-th
        y = torch.stack(y_res, dim=1)
        ...
        return y
class TAB_Layer1d_(nn.Module):
    def __init__(self, num_features, time_steps=250, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True,
                 interval=5):
        super(TAB_Layer1d_, self).__init__()
        self.time_steps = time_steps
        self.interval = interval  # 每隔多少个时间步应用一个 BN 层
        self.num_intervals = (time_steps + interval - 1) // interval
        self.bn_list = nn.ModuleList([nn.BatchNorm1d(num_features, eps, momentum, affine, track_running_stats) for _ in
                                      range(self.num_intervals)])
        self.p = nn.Parameter(torch.ones(self.num_intervals, 1, 1))

    def forward(self, x):
        self.p = (self.p).to(x.device)
        self.bn_list = nn.ModuleList([bn.to(x.device) for bn in self.bn_list])
        pt = _prob_check(self.p)
        ...
        for interval_idx in range(self.num_intervals):
            start = interval_idx * self.interval
            end = min((interval_idx + 1) * self.interval, self.time_steps)

            # Extract the relevant time steps
            x_interval = x[:, start:end, ...].transpose(1, 2).contiguous()  # [N, T, C] -> [N, C, T]

            # Apply Batch Normalization for the interval
            y_interval = self.bn_list[interval_idx](x_interval)
            y_interval = y_interval.transpose(1, 2).contiguous()  # [N, C, T] -> [N, T, C]

            # Collect the results
            y_res.append(y_interval)

        # Stack results for all intervals
        y_res = torch.cat(y_res, dim=1)

        # Reshape and apply weights
        y_res = y_res.transpose(0, 1).contiguous()  # [N, T, C] -> [T, N, C]
        pt_expanded = torch.zeros(self.time_steps, 1, 1).to(x.device)
        for i in range(self.num_intervals):
            start_idx = i * self.interval
            end_idx = min(start_idx + self.interval, self.time_steps)
            pt_expanded[start_idx:end_idx, :, :] = pt[i, :, :]

        # pt_expanded = torch.cat([pt[:self.num_intervals - 1].repeat(self.interval, 1, 1),
        #                      pt[self.num_intervals - 1].repeat(self.time_steps%self.interval, 1, 1)], dim=0)  # [num_intervals, 1, 1] -> [T, 1, 1]
        y = y_res * pt_expanded
        y = y.contiguous().transpose(0, 1)  # [T, N, C] -> [N, T, C]
